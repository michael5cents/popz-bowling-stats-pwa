...
<div class="note"><strong>How to read this:</strong> a device is an anonymous installation/browser profile, not guaranteed to be one unique person. Completed-series devices and repeat activity across multiple days are stronger adoption signals than raw app opens.<br><br><strong>Current baseline caveat:</strong> the early device/open counts include owner QA checks from the public app on at least one personal device, possibly two. Do not treat the current raw device total as outside public adoption.</div>
<div class="panel"><h2>Activation funnel • 30d</h2><p class="muted">New funnel stages begin with public-v30 and cannot be backfilled for earlier visits.</p><table><thead><tr><th>Stage</th><th>Devices</th></tr></thead><tbody>${funnelRows||'<tr><td colspan="2">No funnel telemetry yet.</td></tr>'}</tbody></table></div>\n<div class="panel"><h2>Recent daily activity</h2><table><thead><tr><th>Date</th><th>Devices</th><th>App opens</th><th>Bowl views</th><th>Series started</th><th>First rolls</th><th>Games completed</th><th>Series completed</th></tr></thead><tbody>${rows||'<tr><td colspan="8">No telemetry yet.</td></tr>'}</tbody></table></div>
