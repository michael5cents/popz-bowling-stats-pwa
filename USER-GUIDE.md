...
On the device that already has your history, use **Settings → Share / Transfer Backup**. Choose Quick Share, AirDrop, Google Drive, email, Messages, or another available sharing option. On the new phone or tablet, open Popz Bowling Stats and use **Settings → Import JSON** to select the transferred backup. The backup includes saved games and frame-by-frame history. If the browser cannot share files directly, the app downloads the backup instead so you can send that file manually.
