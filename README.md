...
public-v26 is the first visible **Pro Preview** release. The header and Settings show the effective plan (`Free`, `Pro Preview`, or `Pro`), premium features carry PRO labels, Google Sync is gated, and Advanced Shot Tracking is now implemented behind the same entitlement system. Pro Preview remains enabled, so current testers lose nothing. Pricing and checkout are still undecided. See `PRO-PLAN.md`.
